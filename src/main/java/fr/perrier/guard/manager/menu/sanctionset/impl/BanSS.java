...
public class BanSS extends PaginatedMenu {

    private final Player target;

    @Override
    public String getPrePaginatedTitle(Player player) {
        return Messages.MENU_SANCTIONSET_BAN_TITLE.getMessage().replace("%target%", target.getName());
    }
    ...
    public Map<Integer, Button> getAllPagesButtons(Player player) {
        ...
        for(SanctionSS values : SanctionSS.values()) {
            if(!values.getType().equalsIgnoreCase("Ban")) continue;
            buttons.put(buttons.size(), new BanSanction(values));
        }
    ...
    @RequiredArgsConstructor
    private class BanSanction extends Button {
        ...
        @Override
        public void clicked(@NotNull Player player, int slot, ClickType clickType, int hotbarButton) {
            ...
            try {
                ...
                }, getSanctionItem(value), new BanSS(target)).openMenu(player);
            }catch (Exception e1) {
                player.sendMessage(ChatUtil.translate(Messages.prefix() + "&4ERR0R please contact support with this ID: [SS-BAN~L.80]"));
                e1.printStackTrace();
            }
        }
    }
    ...
    public static ItemStack getSanctionItem(SanctionSS value) {
        ItemBuilder itemBuilder = new ItemBuilder(value.getItem());
        itemBuilder.setName("&f" + value.getReason());

        for (String lore : Messages.MENU_SANCTIONSET_BAN_LORE.getLore()) {
