...
public class WarnSS extends PaginatedMenu {

    private final Player target;

    @Override
    public String getPrePaginatedTitle(Player player) {
        return Messages.MENU_SANCTIONSET_WARN_TITLE.getMessage().replace("%target%", target.getName());
    }
    ...
    public Map<Integer, Button> getAllPagesButtons(Player player) {
        ...
        for(SanctionSS values : SanctionSS.values()) {
            if(!values.getType().equalsIgnoreCase("Warn")) continue;
            buttons.put(buttons.size(),new WarnSanction(values));
        }
    ...
    @RequiredArgsConstructor
    private class WarnSanction extends Button {
        ...
        public void clicked(Player player, int slot, ClickType clickType, int hotbarButton) {
            ...
            try {
                new ConfirmationMenu(() -> {
                    String sanction = Guard.getDefaultConfig().getString("sanction.warn");
                    ...
                    player.closeInventory();
                }, getSanctionItem(value), new WarnSS(target)).openMenu(player);
            }catch (Exception e1) {
                player.sendMessage(ChatUtil.translate(Messages.prefix() + "&4ERR0R please contact support with this ID: [SS-WARN~L.78]"));
                e1.printStackTrace();
            }
        }
    }
    ...
    public static ItemStack getSanctionItem(SanctionSS value) {
        ...
        for(String lore : Messages.MENU_SANCTIONSET_WARN_LORE.getLore()) {
